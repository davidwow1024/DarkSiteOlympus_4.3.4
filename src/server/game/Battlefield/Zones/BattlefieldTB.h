...
struct BfTBGameObjectBuilding
{
    ...
    void Damaged()
    {
        ...
        if (m_NameId)
            m_TB->SendWarningToAllInWar(BATTLEFIELD_TB_TEXT_TOWER_DAMAGE, sObjectMgr->GetOlympusStringForDBCLocale(m_NameId));
    }
    ...
    void Destroyed()
    {
        ...
        // Warning
        if (m_NameId)
            m_TB->SendWarningToAllInWar(BATTLEFIELD_TB_TEXT_TOWER_DESTROY, sObjectMgr->GetOlympusStringForDBCLocale(m_NameId));
    ...
    void GiveControlTo(uint8 team, bool init /* for first call in setup*/)
    {
        ...
        switch (team)
        {
            case TEAM_NEUTRAL:
            {
                ...
                if (!init)
                    m_TB->SendWarningToAllInWar(BATTLEFIELD_TB_TEXT_FORT_ATTACK, sObjectMgr->GetOlympusStringForDBCLocale(m_NameId),
                                                sObjectMgr->GetOlympusStringForDBCLocale(m_TeamControl ? BATTLEFIELD_TB_TEXT_ALLIANCE : BATTLEFIELD_TB_TEXT_HORDE));
            ...
            case TEAM_ALLIANCE:
            {
                ...
                // Warning message
                if (!init)
                    m_TB->SendWarningToAllInWar(BATTLEFIELD_TB_TEXT_FORT_TAKEN, sObjectMgr->GetOlympusStringForDBCLocale(m_NameId),
                                                sObjectMgr->GetOlympusStringForDBCLocale(BATTLEFIELD_TB_TEXT_ALLIANCE));
            ...
            case TEAM_HORDE:
            {
                ...
                if (!init)
                    m_TB->SendWarningToAllInWar(BATTLEFIELD_TB_TEXT_FORT_TAKEN, sObjectMgr->GetOlympusStringForDBCLocale(m_NameId),
                                                sObjectMgr->GetOlympusStringForDBCLocale(BATTLEFIELD_TB_TEXT_HORDE));

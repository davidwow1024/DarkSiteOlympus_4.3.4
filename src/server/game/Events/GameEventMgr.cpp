...
void GameEventMgr::RunSmartAIScripts(uint16 event_id, bool activate)
{
	...
	{
		OLYMPUS_READ_GUARD(HashMapHolder<Creature>::LockType, *HashMapHolder<Creature>::GetLock());
	...
	{
	OLYMPUS_READ_GUARD(HashMapHolder<GameObject>::LockType, *HashMapHolder<GameObject>::GetLock());

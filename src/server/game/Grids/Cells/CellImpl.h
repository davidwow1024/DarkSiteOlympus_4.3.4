...
#ifndef OLYMPUS_CELLIMPL_H
#define OLYMPUS_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = Olympus::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = Olympus::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = Olympus::ComputeCellCoord(x + radius, y + radius).normalize();

...
public:
    ...
    {
        if (sConfigMgr->GetBoolDefault("AOELoot.Enable", true))
        {
    ...
    {
        bool _enable = sConfigMgr->GetBoolDefault("AOELoot.Enable", true);
        ...
        for (auto const& _creature : creaturedie)
        {
            ...
            for (uint32 i = 0; i < maxSlot; ++i)
            {
                if (LootItem* item = loot->LootItemInSlot(i, player))
                {
                    ...
                    if (itemTemplate->MaxCount != 1)
                    {
                        ...
                        else if (sConfigMgr->GetBoolDefault("AOELoot.MailEnable", true))
                        {
            ...
            if (!loot->empty())
            {
                if (!_creature->isAlive())
                {
                    ...
                    if (_creature->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE))
                    {
                        _creature->RemoveFlag(UNIT_DYNAMIC_FLAGS, UNIT_FLAG_SKINNABLE);
                    }
                }
            }
            else

...
public:
    ...
    Unit* GetTargetByType(uint8 uiType, float fRange, float fArc = M_PI)
    {
        ...
        return Olympus::Containers::SelectRandomContainerElement(vTargets);
    }
    ...
    Unit* GetMovementSquare()
    {
        ...
        lEnemies.sort(Olympus::ObjectDistanceOrderPred(me));
        lSquaresList.sort(Olympus::ObjectDistanceOrderPred(lEnemies.front()));
...
public:
    ...
    class spell_medivh_cheat_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            if (targets.size() > 3)
                Olympus::Containers::RandomResizeList(targets, 3);
        }

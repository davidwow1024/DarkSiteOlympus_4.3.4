    ...
    static bool HandleLookupEventCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < events.size(); ++id)
        {
            ...
            if (Utf8FitTo(descr, wNamePart))
            {
                ...
                char const* active = activeEvents.find(id) != activeEvents.end() ? handler->GetOlympusString(LANG_ACTIVE) : "";
        ...
        for (uint32 id = 0; id < sFactionStore.GetNumRows(); ++id)
        {
            if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(id))
            {
                ...
                if (factionState) // and then target != NULL also
                {
                    ...
                    std::string rankName = handler->GetOlympusString(index);

                    ss << ' ' << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ')';

                    if (factionState->Flags & FACTION_FLAG_VISIBLE)
                        ss << handler->GetOlympusString(LANG_FACTION_VISIBLE);
                    if (factionState->Flags & FACTION_FLAG_AT_WAR)
                        ss << handler->GetOlympusString(LANG_FACTION_ATWAR);
                    if (factionState->Flags & FACTION_FLAG_PEACE_FORCED)
                        ss << handler->GetOlympusString(LANG_FACTION_PEACE_FORCED);
                    if (factionState->Flags & FACTION_FLAG_HIDDEN)
                        ss << handler->GetOlympusString(LANG_FACTION_HIDDEN);
                    if (factionState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                        ss << handler->GetOlympusString(LANG_FACTION_INVISIBLE_FORCED);
                    if (factionState->Flags & FACTION_FLAG_INACTIVE)
                        ss << handler->GetOlympusString(LANG_FACTION_INACTIVE);
                }
                else
                    ss << handler->GetOlympusString(LANG_FACTION_NOREPUTATION);
                            ...
                            if (target)
                            {
                                ...
                                switch (status)
                                {
                                    case QUEST_STATUS_COMPLETE:
                                        statusStr = handler->GetOlympusString(LANG_COMMAND_QUEST_COMPLETE);
                                        break;
                                    case QUEST_STATUS_INCOMPLETE:
                                        statusStr = handler->GetOlympusString(LANG_COMMAND_QUEST_ACTIVE);
                                        break;
                                    case QUEST_STATUS_REWARDED:
                                        statusStr = handler->GetOlympusString(LANG_COMMAND_QUEST_REWARDED);
                                        break;
                                    default:
                                        break;
                                }
    ...
    static bool HandleLookupSkillCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
        {
            ...
            if (skillInfo)
            {
                ...
                {
                    knownStr = handler->GetOlympusString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue  = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = handler->GetOlympusString(LANG_SKILL_VALUES);
    ...
    static bool HandleLookupSpellCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sSpellMgr->GetSpellInfoStoreSize(); id++)
        {
            ...
            if (spellInfo)
            {
                ...
                if (rank)
                    ss << handler->GetOlympusString(LANG_SPELL_RANK) << rank;

                if (handler->GetSession())
                    ss << "]|h|r";

                if (talent)
                    ss << handler->GetOlympusString(LANG_TALENT);
                if (passive)
                    ss << handler->GetOlympusString(LANG_PASSIVE);
                if (learn)
                    ss << handler->GetOlympusString(LANG_LEARN);
                if (known)
                    ss << handler->GetOlympusString(LANG_KNOWN);
                if (active)
                    ss << handler->GetOlympusString(LANG_ACTIVE);
    ...
    static bool HandleLookupTitleCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
        {
            ...
            if (titleInfo)
            {
                ...
                char const* knownStr = target && target->HasTitle(titleInfo) ? handler->GetOlympusString(LANG_KNOWN) : "";

                char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->bit_index
                    ? handler->GetOlympusString(LANG_ACTIVE)
                    : "";
            ...
            if (MapEntry const* mapInfo = sMapStore.LookupEntry(id))
            {
                ...
                if (Utf8FitTo(name, wNamePart) && locale < TOTAL_LOCALES)
                {
                    ...
                    if (mapInfo->IsContinent())
                        ss << handler->GetOlympusString(LANG_CONTINENT);

                    switch (mapInfo->map_type)
                    {
                        case MAP_INSTANCE:
                            ss << handler->GetOlympusString(LANG_INSTANCE);
                            break;
                        case MAP_RAID:
                            ss << handler->GetOlympusString(LANG_RAID);
                            break;
                        case MAP_BATTLEGROUND:
                            ss << handler->GetOlympusString(LANG_BATTLEGROUND);
                            break;
                        case MAP_ARENA:
                            ss << handler->GetOlympusString(LANG_ARENA);
                            break;
                    }

...
public:
    ...
    static bool HandleDebugEnterVehicleCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            Olympus::AllCreaturesOfEntryInRange check(handler->GetSession()->GetPlayer(), entry, 20.0f);
            Olympus::CreatureSearcher<Olympus::AllCreaturesOfEntryInRange> searcher(handler->GetSession()->GetPlayer(), passenger, check);

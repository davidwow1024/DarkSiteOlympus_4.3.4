...
public:
    ...
    static bool HandleAnnounceCommand(ChatHandler* handler, char const* args)
    {
        ...
        char buff[2048];
        sprintf(buff, handler->GetOlympusString(LANG_SYSTEMMESSAGE), args);
    ...
    static bool HandleNotifyCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string str = handler->GetOlympusString(LANG_GLOBAL_NOTIFY);
        str += args;
    ...
    static bool HandleGMNotifyCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string str = handler->GetOlympusString(LANG_GM_NOTIFY);
        str += args;
    ...
    static bool HandleWhispersCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, handler->GetSession()->GetPlayer()->isAcceptWhispers() ?  handler->GetOlympusString(LANG_ON) : handler->GetOlympusString(LANG_OFF));
            return true;
        }

...
public:
    ...
    void OnLogout(Player* player)
    {
        if (player->GetCurrency(CURRENCY_TYPE_CONQUEST_POINTS, true))
        {
            ...
            uint32 capArena = Olympus::Currency::ConquestRatingCalculator(player->GetMaxPersonalArenaRatingRequirement(0)) * CURRENCY_PRECISION;
            uint32 capRBG = Olympus::Currency::BgConquestRatingCalculator(player->GetRbgOrSoloQueueRatingForCapCalculation()) * CURRENCY_PRECISION;

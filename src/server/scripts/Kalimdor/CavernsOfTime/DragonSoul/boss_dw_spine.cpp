...
public:
    ...
    struct boss_dw_spineAI : public BossAI
    {
        ...
        void ActivateNextCorruption(std::list<Creature*> spawnerList, bool first)
        {
            ...
            spawnerList.sort(Olympus::ObjectDistanceOrderPred(me));
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SUMMON_CORRUPTED_BLOOD:
                    {
                        ...
                        {
                            Unit* target = Olympus::Containers::SelectRandomContainerElement(spawnerList);
                            if (target)
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (!corruption)
            {
                ...
                if (!creatureList.empty())
                    corruption = Olympus::Containers::SelectRandomContainerElement(creatureList);
...
public:
    ...
    class spell_ds_searing_plasma_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            targets.remove_if(Olympus::UnitAuraCheck(true, sSpellMgr->GetSpellIdForDifficulty(SPELL_SEARING_PLASMA_TR, GetCaster())));
            if (!targets.empty())
...
public:
    ...
    class spell_ds_blood_corruption_aura_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targetlist)
        {
            ...
            targetlist.remove_if(Olympus::UnitAuraCheck(true, GetSpellInfo()->Id == SPELL_BLOOD_CORRUPTION_DEATH ? SPELL_BLOOD_CORRUPTION_EARTH : SPELL_BLOOD_CORRUPTION_DEATH));
            targetlist.remove(GetCaster());
            if (!targetlist.empty())
                Olympus::Containers::RandomResizeList(targetlist, count);
        }

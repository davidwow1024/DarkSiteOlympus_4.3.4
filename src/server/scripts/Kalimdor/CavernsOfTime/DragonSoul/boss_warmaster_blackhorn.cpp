...
public:
    ...
    struct npc_ds_sky_captain_swayzeAI : public Scripted_NoMovementAI
    {
        ...
        void setCannonsCombatState(bool active, bool phaseTwo = false)
        {
            ...
            Olympus::AllWorldObjectsInRange objects(me, 70.0f);
        ...
        void SpawnDeckFire(bool initial)
        {
            if (initial)
            {
                if (Creature* deckFireController = me->SummonCreature(NPC_DECK_FIRE_CONTROLLER, deckFireSpawnPos, TEMPSUMMON_MANUAL_DESPAWN))
                {
                    ...
                    if (!triggers.empty())
                    {
                        auto trigger = Olympus::Containers::SelectRandomContainerElement(triggers);
                        Position pos;
                            ...
                            for (int8 i = 0; i < 2; i++)
                            {
                                if (DynamicObject* lastDynObj = summon->GetLastDynObject())
                                {
                                    ...
                                    for (uint8 i = 0; i < 16; ++i)
                                    {
                                        ...
                                        std::list<WorldObject*> targetList;
                                        Olympus::AllWorldObjectsInRange objects(lastDynObj, 8.0f);
                                        Olympus::WorldObjectListSearcher<Olympus::AllWorldObjectsInRange> searcher(lastDynObj, targetList, objects, GRID_MAP_TYPE_MASK_DYNAMICOBJECT);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_MASSIVE_EXPLOSION:
                        if (Creature* explosion = Olympus::Containers::SelectRandomContainerElement(massiveExplosion))
                    ...
                    case EVENT_CHECK_BRIGADE_TARGET:
                    {
                        ...
                        Olympus::WorldObjectListSearcher<Olympus::AllWorldObjectsInRange> searcher(me, dynObjList, objects, GRID_MAP_TYPE_MASK_DYNAMICOBJECT);
...
public:
    ...
    class spell_ds_consuming_shroud_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            targets.remove_if(Olympus::UnitAuraCheck(true, sSpellMgr->GetSpellIdForDifficulty(SPELL_CONSUMING_SHROUD, GetCaster())));
...
struct DeckFireTargetCheck : public Olympus::WorldObjectSpellAreaTargetCheck
{
    DeckFireTargetCheck(float range, Position const* position, Unit* caster, Unit* referer, SpellInfo const* spellInfo)
        : Olympus::WorldObjectSpellAreaTargetCheck(range, position, caster, referer, spellInfo, TARGET_CHECK_ALLY, nullptr)
    {}

    bool operator()(Unit* target)
    {
        if (!Olympus::WorldObjectSpellAreaTargetCheck::operator()(target))
            return false;
...
public:
    ...
    class spell_ds_deck_fire_periodic_AuraScript : public AuraScript
    {
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            Olympus::UnitListSearcher<DeckFireTargetCheck> searcher(me, targets, check);

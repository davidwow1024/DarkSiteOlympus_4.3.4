...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    Olympus::AllFriendlyCreaturesInGrid creature_check(me);
    Olympus::CreatureListSearcher<Olympus::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

    TypeContainerVisitor <Olympus::CreatureListSearcher<Olympus::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    Olympus::WorldObjectWorker<Olympus::RespawnDo> worker(me, u_do);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        CellCoord pair(Olympus::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
        Cell cell(pair);
        cell.SetNoCreate();
        ...
        TypeContainerVisitor
            <Olympus::CreatureListSearcher<Olympus::AllFriendlyCreaturesInGrid>,

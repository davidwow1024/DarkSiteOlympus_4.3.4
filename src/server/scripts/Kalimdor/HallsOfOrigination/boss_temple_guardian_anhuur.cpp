    ...
    struct boss_temple_guardian_anhuurAI : public BossAI
    {
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage)
        {
                ...
                (me->HealthBelowPctDamaged(33, damage) && _phase == PHASE_SECOND_SHIELD))
            {
                ...
                stalkers.remove_if(Olympus::HeightDifferenceCheck(door, 0.0f, false)); // Target only the bottom ones
                for (std::list<Creature*>::iterator itr = stalkers.begin(); itr != stalkers.end(); ++itr)
                {
        ...
        void DoAction(int32 const action)
        {
            if (action == ACTION_CHECK_BEACONS)
            {
                ...
                GameObject* door = ObjectAccessor::GetGameObject(*me, instance->GetData64(DATA_ANHUUR_DOOR));
                GetCreatureListWithEntryInGrid(stalkers, me, NPC_CAVE_IN_STALKER, 100.0f);
                stalkers.remove_if(Olympus::HeightDifferenceCheck(door, 0.0f, false));
                for (std::list<Creature*>::iterator itr = stalkers.begin(); itr != stalkers.end(); ++itr)
                {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SEAR:
                    {
                        ...
                        std::list<Creature*> stalkers;
                        GetCreatureListWithEntryInGrid(stalkers, me, NPC_CAVE_IN_STALKER, 100.0f);
                        stalkers.remove_if(Olympus::HeightDifferenceCheck(ObjectAccessor::GetGameObject(*me, instance->GetData64(DATA_ANHUUR_DOOR)), 5.0f, true));

                        if (stalkers.size() < 2)
                            break;

                        stalkers.sort(Olympus::ObjectDistanceOrderPred(target));

                        // Get the closest statue face (any of its eyes)
                        Creature* eye1 = stalkers.front();
                        stalkers.remove(eye1); // Remove the eye.
                        stalkers.sort(Olympus::ObjectDistanceOrderPred(eye1)); // Find the second eye.
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                if (InstanceMap* instance = GetCaster()->GetMap()->ToInstanceMap())
                {
                    if (InstanceScript* const script = instance->GetInstanceScript())
                    {
                        ...
                        {
                            targets.remove_if(Olympus::HeightDifferenceCheck(go, 5.0f, false));

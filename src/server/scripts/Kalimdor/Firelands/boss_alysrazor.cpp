...
static void AlysrazorTrashEvaded(Creature *creature)
{
    ...
    Olympus::CreatureWorker<TrashRespawnWorker> worker(creature, check);
...
public:
    ...
    class spell_alysrazor_blazing_power_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject *> &targets)
        {
            ...
            targets.remove_if(Olympus::UnitAuraCheck(true, SPELL_BLAZING_POWER_PREVENTION_AURA));
...
public:
    ...
    class spell_alysrazor_imprinted_targeting_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject *> &targets)
        {
            targets.remove_if(Olympus::UnitAuraCheck(true, SPELL_IMPRINTED));
            targets.remove_if(Olympus::UnitAuraCheck(true, SPELL_IMPRINTED_2));
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            if (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_SUMMON_SMOULDERING_HATCHLING:
                {
                    ...
                    Olympus::CreatureListSearcher<MoltenEggCheck> searcher(me, eggs, check);
                    me->VisitNearbyGridObject(20.0f, searcher);
                    if (!eggs.empty())
                    {
                        Creature *egg = Olympus::Containers::SelectRandomContainerElement(eggs);

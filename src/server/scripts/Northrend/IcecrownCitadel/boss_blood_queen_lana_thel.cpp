/*
 * Copyright (C) 2008-2012 OlympusCore <http://www.OlympusCore.org/>
 *
    ...
    public:
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_PACT_OF_THE_DARKFALLEN:
                        {
                            ...
                            if (Is25ManRaid())
                                ++targetCount;
                            Olympus::Containers::RandomResizeList<Player*>(targets, targetCount);
                            if (targets.size() > 1)
                            {
                        ...
                        case EVENT_TWILIGHT_BLOODBOLT:
                        {
                            std::list<Player*> targets;
                            SelectRandomTarget(false, &targets);
                            Olympus::Containers::RandomResizeList<Player*>(targets, uint32(Is25ManRaid() ? 4 : 2));
                            for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
        ...
        private:
            ...
            Player* SelectRandomTarget(bool includeOfftank, std::list<Player*>* targetList = NULL)
            {
                ...
                if (includeOfftank)
                {
                    tempTargets.sort(Olympus::ObjectDistanceOrderPred(me->getVictim()));
...
class spell_blood_queen_pact_of_the_darkfallen : public SpellScriptLoader
{
    public:
        ...
        class spell_blood_queen_pact_of_the_darkfallen_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                targets.remove_if(Olympus::UnitAuraCheck(false, SPELL_PACT_OF_THE_DARKFALLEN));
...
class spell_blood_queen_pact_of_the_darkfallen_dmg_target : public SpellScriptLoader
{
    public:
        ...
        class spell_blood_queen_pact_of_the_darkfallen_dmg_SpellScript : public SpellScript
        {
            ...
            {
                unitList.remove_if(Olympus::UnitAuraCheck(true, SPELL_PACT_OF_THE_DARKFALLEN));

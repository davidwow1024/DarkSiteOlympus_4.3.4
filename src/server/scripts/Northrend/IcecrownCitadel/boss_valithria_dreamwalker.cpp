...
class ValithriaDespawner : public BasicEvent
{
    public:
        ...
        {
            Olympus::CreatureWorker<ValithriaDespawner> worker(_creature, *this);
        ...
        struct npc_risen_archmageAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit* /*target*/)
            {
                ...
                if (me->GetDBTableGUIDLow() && _canCallEnterCombat)
                {
                    ...
                    Olympus::CreatureListSearcher<RisenArchmageCheck> searcher(me, archmages, check);
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            Player* player = NULL;
                            Olympus::AnyPlayerInObjectRangeCheck check(me, 5.0f);
...
class spell_dreamwalker_summoner : public SpellScriptLoader
{
    public:
        ...
        class spell_dreamwalker_summoner_SpellScript : public SpellScript
        {
            ...
            {
                targets.remove_if(Olympus::UnitAuraCheck(true, SPELL_RECENTLY_SPAWNED));
                if (targets.empty())
                    return;
    ...
    public:
        ...
        class spell_dreamwalker_summon_suppresser_AuraScript : public AuraScript
        {
            ...
            void PeriodicTick(AuraEffect const* /*aurEff*/)
            {
                ...
                summoners.remove_if(Olympus::UnitAuraCheck(true, SPELL_RECENTLY_SPAWNED));
                Olympus::Containers::RandomResizeList(summoners, 2);
                if (summoners.empty())
                    return;

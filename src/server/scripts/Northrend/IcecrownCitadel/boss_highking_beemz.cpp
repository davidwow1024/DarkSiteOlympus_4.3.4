...
public:
    ...
    struct boss_gunship_ersatzAI : public BossAI
    {
        ...
        void SummonAdds()
        {
            ...
            Olympus::Containers::RandomResizeList(entries, uint32(IsHeroic() ? 4 : 3));
            uint8 i = 0;
        ...
        void SummonAllAdds()
        {
            ...
            Olympus::Containers::RandomResizeList(entries, 10);
            uint8 i = 0;
            for (std::list<uint32>::const_iterator itr = entries.begin(); itr != entries.end(); ++itr, ++i)
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_FIRE:
                    {
                        ...
                        if (Is25ManRaid())
                            targetCount += 2;
                        Olympus::Containers::RandomResizeList<Player*>(targets, uint32(Is25ManRaid() ? 6 : 2));
                        for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)

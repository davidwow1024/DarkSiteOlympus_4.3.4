/* Copyright (C) 2008 - 2009 Olympus <http://www.OlympusCore.org/>
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            for (uint8 i = 0; i < graveyard.size(); i++)
                if (graveyard[i]->GetControlTeamId() == player->GetTeamId())
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GetOlympusStringForDBCLocale(((BfGraveyardWG*)graveyard[i])->GetTextId()), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + i);
...
class npc_wg_queue : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GetOlympusStringForDBCLocale(WG_NPCQUEUE_TEXTOPTION_JOIN), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
